...
/**
 * Value of a unification variable. We implement Tarjan's union-find
 * algorithm: when two variables are unified, one of them is converted
...
/**
 * Table of unification variables and their values.
 */
pub struct UnificationTable<K,V> {
    /** Indicates the current value of each variable. */
 ...
 * Conveys the current root and value of the variable.
 */
pub struct Node<K,V> {
    pub key: K,
    pub value: V,
...
impl<V:PartialEq+Clone+Repr,K:UnifyKey<V>> UnificationTable<K,V> {
     ...
     * removes any variables that have been created since then.
     */
